...
_parse_kafka_brokers(const char *token,char *cptr){
    ...
    DEBUGMSGTL(("kafka:brokers:parse", "brokers set to %s\n", _kafka.brokers));
...
static void produce (void *buf, size_t len, int msgflags) {
    int retried = 0;
    rd_kafka_resp_err_t rkerr; /* Declaration moved */
    ...
    do {
        ...
        rkerr = rd_kafka_errno2err(errno); /* Assignment after declaration */
...
netsnmp_kafka_init(void)
{
    ...
    DEBUGMSGTL(("kafka:init","_kafka.brokers at init: %s\n", _kafka.brokers ? _kafka.brokers : "(null)"));
    ...
    if (rd_kafka_conf_set(conf, "bootstrap.servers", _kafka.brokers, errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
        snmp_log(LOG_ERR, "Failed to set librdkafka configuration \"bootstrap.servers\": %s\n", errstr);
        return -1;
    }
...
static int strbuffer_format_trap(strbuffer_t *buffer,const char *attribute_name,const char *format, netsnmp_pdu *pdu,netsnmp_transport *transport){
    ...
    strbuffer_append(buffer,"\"");
    rc = realloc_format_trap((u_char **)&buffer->value,&buffer->buf_len,&buffer->buf_out,1,format,pdu,transport);
...
static void transport2buffer(strbuffer_t *buffer,const char *attr_name,netsnmp_pdu *pdu,netsnmp_transport *transport){
    ...
    char * str_transport;
    print_attr_name(buffer,attr_name);
    initial_length = buffer->buf_out;

    str_transport = transport->f_fmtaddr(transport, pdu->transport_data,pdu->transport_data_length);
    if(transport){
        strbuffer_append(buffer,"\"");
...
static size_t print_var_oid_as_json_key(strbuffer_t *buffer,netsnmp_variable_list *var){
    ...
    strbuffer_append(buffer,"\""); // Closing quote for the key
    strbuffer_append(buffer,":"); // Colon separator
...
static size_t print_var_value_as_json_value(strbuffer_t *buffer,netsnmp_variable_list *var){
    ...
    if(_have_to_add_quotes)
        strbuffer_append(buffer,"\"");
    value_rc = sprint_realloc_by_type((u_char**)&buffer->value, &buffer->buf_len,
...
static int varbind2strbuffer(strbuffer_t *buffer,netsnmp_pdu *pdu){
    ...
    while(var){
        ...
        bytes_writted = var2strbuffer(buffer,var);
        if(bytes_writted <= 0){
            buffer->buf_out = initial_length;
            buffer->value[buffer->buf_out] = '\0';
        }
